...
export default function SentimentScore({
  ...
  const positiveCount = sentimentData?.length
    ? sentimentData?.filter((item) => item.label === 'Positive').length
    : 0;

  const negativeCount = sentimentData?.length
    ? sentimentData?.filter((item) => item.label === 'Negative').length
    : 0;

  const score =
    ((positiveCount - negativeCount) * 100) / (sentimentData?.length || 0);
