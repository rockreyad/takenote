...
function DarkIcon(props: { className: string }) {
  return (
    <svg aria-hidden="true" viewBox="0 0 16 16" {...props}>
      <path
        fillRule="evenodd"
...
export function ThemeSelector(props: { className?: string }) {
  ...
  useEffect(() => {
    if (theme) {
      const theTheme = themes.find((t) => t.value === theme);
      setSelectedTheme({
        name: theTheme?.name ?? 'System',
        value: theTheme?.value ?? 'system',
        icon: theTheme?.icon ?? SystemIcon
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
